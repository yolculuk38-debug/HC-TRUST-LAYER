...
Treat the Actions summary and artifacts as the current public-safe evidence view, not as private/admin-only notifications. The future same-repo fixed issue is documented in [HC Signal Watch Same-Repo Console Mode](github-signal-watch-same-repo-console-mode.md), but it is not implemented here. No second repository is required for the current model. Optional private/admin-only notifications remain future and parked, as described in [HC Signal Watch Private Inbox Setup Contract](github-signal-watch-private-inbox-setup.md).
