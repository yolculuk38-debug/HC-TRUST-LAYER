...
| --- | --- | --- | --- |
| A record normalization script silently rewrites records. | Not confirmed in this triage | `normalize_records.py` was not found at repository root or by content search during the current checks. Search results only showed documentation/workflow references. | Inspect archive workflow references before opening any fix PR. Do not create a fix for a file that is not present on main. |
