...
| Field | Status |
| --- | --- |
| Current phase | Repository cleanup phase 1 mapping, the explicitly approved 2026-07-22 branch cleanup, the bundled Public Validator `record_id` runner alignment, the first scannable demo QR entry, the live navigation/fail-closed RED TEAM hardening, and the task-handoff test-contract repair are complete through #1220. The branch operation deleted 76 verified stale branches and preserved all 36 hold branches. Phase 2 and later Public Validator slices must stay small, reversible, and evidence-backed. |
| Active focus | Expose the existing combined local QR/Public Validator engine through one deterministic local CLI entry point while keeping the three intentional open issue surfaces distinct: #812 assistant console, #1082 Signal Watch console, and #1109 Mission Control. Completed verification, HC Council local runner, QR compatibility repair, planner, inventory, governance, cleanup phase 1, branch cleanup, demo QR entry, live RED TEAM acceptance, and task-handoff test repair are historical references, not active work. |
| Next up | Add one local-only CLI wrapper for `hc_runtime.qr_public_validator.run_qr_public_validator`, with deterministic JSON output and focused valid, mismatch, malformed, and no-network tests. Reuse the existing engine without adding a backend, remote fetch, signing, QR-authenticity, truth, or production-authority claims. |
...
- The Public Validator runner `record_id` contract, first scannable demo QR entry, README/START_HERE navigation, fail-closed selector handling, and live mobile RED TEAM acceptance are complete through #1216-#1219. #1220 restored the full green test baseline. The next bounded implementation is a local CLI for the existing combined QR/Public Validator engine, not another demo QR entry.
...
The immediate safe task is a small local CLI wrapper for the existing `hc_runtime.qr_public_validator.run_qr_public_validator` engine. Accept one local QR payload JSON argument, print deterministic sorted JSON with no extra stdout prose, preserve `advisory_only=true`, `public_safe=true`, `truth_guarantee=false`, and `human_review_required=true`, and add focused CLI tests for valid, mismatch, malformed, and no-network behavior. Do not change the engine contract, lookup allowlist, schemas, validators, records, workflows, signing, canonical URL handling, or network posture. The 36 held branches and any later branch remain outside cleanup authority without new evidence and explicit human approval.
