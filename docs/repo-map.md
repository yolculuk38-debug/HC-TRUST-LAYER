...
| `docs/workflows/` | Workflow documentation and cleanup planning. Actual workflow mutations require separate human-approved PRs. | documentation/planning | Maintainers and contributors may update scoped docs-only planning. | Medium to high: workflow documentation can affect automation interpretation, but does not change automation. |
...
| `docs/governance/canonical-vs-advisory-governance-surfaces.md` | Conservative reading guide for canonical, advisory, project-control, historical/reference, and onboarding governance surfaces. | governance | Maintainers and governance reviewers. | High: documents interpretation boundaries without changing authority. |
...
| `council/`, `reviewers/`, `witness/`, `timeline/`, `witness-archive/` | Governance, reviewer, witness, timeline, and provenance evidence surfaces. | audit/provenance record | Maintainers; preserve provenance while using current HC:// and HC-TRUST-LAYER language for normal repository content. | High: evidence surfaces; witness archive wording may be normalized only when explicitly scoped and professionally bounded. |
...
- **Evidence and provenance surfaces:** records, archived materials, witness/timeline material, and provenance examples preserve audit history. Normal repository content should use professional English and current HC:// / HC-TRUST-LAYER identity; witness archive records may be normalized to current terminology when explicitly scoped. Do not claim endorsement, legal identity verification, forensic certainty, production verification, or truth finality.
