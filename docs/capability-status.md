...
| QR payload inspection | Partial | `src/qr.py`, `src/qr_hardening.py`, `tests/test_qr_hardening.py`, `docs/demo-flow.md` | Public Verification | QR generation plus local structure, content-hash, and URL checks are present. Signature verification, issuer authentication, QR authenticity, and truth verification are not implemented. |
