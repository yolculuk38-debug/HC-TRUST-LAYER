...
PR **#658 Local Public Validator golden output fixtures** adds illustrative JSON fixtures for the local CLI `found`, `not_found`, and `invalid_record_id` result shapes under `docs/demo/fixtures/local-validator-output/`. These fixtures preserve the stable result contract and safety markers without changing runtime behavior, schemas, validators, workflows, lookup logic, canonical records, QR/signing/federation behavior, or production-readiness boundaries. Human review remains required.
